...
contract CroDefiSwapFactory is ICroDefiSwapFactory {
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        ...
        allPairs.push(pair); emit PairCreated(token0, token1, pair, allPairs.length); }
    ...
    function setFeeSetter(address _feeSetter) external { require(msg.sender == feeSetter, 'CroDefiSwap: FORBIDDEN - only current feeSetter can update next feeSetter');
        address previousFeeSetter = feeSetter;
        feeSetter = _feeSetter; emit FeeSetterUpdated(feeSetter, previousFeeSetter);  }
        function setFeeToBasisPoint(uint _feeToBasisPoint) external { require(msg.sender == feeSetter, 'CroDefiSwap: FORBIDDEN - only current feeSetter can update feeToBasisPoint');
